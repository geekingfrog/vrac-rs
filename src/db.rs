...
pub fn create_token(conn: &mut SqliteConnection, tok: CreateToken) -> Result<Token> {
    ...
    let existing_count: i64 = token::table
        ...
        .first(conn)?;
    if existing_count > 0 {
        bail!("A valid token already exists for the path {}", tok.path)
    };
...
/// Mark the given as Used, files have been uploaded
