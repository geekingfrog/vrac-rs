...
async fn main() -> Result<()> {
    ...
    let service = Arc::new(db::DB::new("./vrac.sqlite"));
    service.init_db()?;

    // let tok = db::CreateToken{
    //     path: "test/path".to_string(),
    //     max_size_in_mb: Some(1024),
    //     token_expires_at: chrono::Utc::now().naive_utc(),
    //     content_expires_at: None,
    // };
    //
    // let created_token = service.create_token(tok)?;
    // println!("created token: {:?}", created_token);
    ...
    warp::serve(
        token_routes.recover(handle_rejection)
...
#[derive(Debug)]
struct VracErr(anyhow::Error);

impl warp::reject::Reject for VracErr {}

fn into_reject(err: anyhow::Error) -> warp::Rejection {
    warp::reject::custom(VracErr(err))
}

async fn create_token(token: api::Token, db: Arc<db::DB>) -> Result<impl Reply, warp::Rejection> {
    let db_token = to_db_token(token);
    let db_token =
        tokio::task::block_in_place(move || db.create_token(db_token)).map_err(into_reject)?;

    let uri = Uri::builder()
        .path_and_query(&format!("/f/{}", db_token.path)[..])
        .build()
        .map_err(|e| into_reject(e.into()))?;
...
fn to_db_token(tok: api::Token) -> db::CreateToken {
    ...
    let tmp: Option<Duration> = tok.content_expires_after.into();
    let content_expires_at = tmp.map(|x| now + x);
